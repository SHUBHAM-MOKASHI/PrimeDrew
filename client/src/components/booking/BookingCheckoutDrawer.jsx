...
export const BookingCheckoutDrawer = ({ isOpen, onClose, vehicle }) => {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={bookingSuccess ? 'Booking Request Confirmed!' : `Reserve ${vehicle.title}`}
      maxWidth="max-w-lg"
    >
      {bookingSuccess ? (
        <div className="flex flex-col items-center text-center gap-4 py-4 animate-in zoom-in-95 duration-200">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900">Trip Requested Successfully!</h3>
            <p className="text-xs text-slate-500 mt-1">
              Your payment of <span className="font-bold text-slate-800">₹{totalPayable}</span> is locked securely in Escrow until host approves key handover.
            </p>
          </div>

          <div className="w-full bg-slate-50 border border-slate-100 rounded-xl p-4 text-left text-xs space-y-2">
            ...
            <div className="flex justify-between text-slate-600">
              ...
              <span className="font-semibold text-slate-800">{new Date(startDate).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-slate-600">
              <span>Dropoff Window:</span>
              <span className="font-semibold text-slate-800">{new Date(endDate).toLocaleString()}</span>
            </div>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-4 bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
            ...
            />
            <div>
              <h4 className="text-sm font-bold text-slate-900">{vehicle.title}</h4>
              <p className="text-xs text-slate-500">{vehicle.category} • {vehicle.specs?.transmission || vehicle.transmission || 'Automatic'}</p>
          ...
          {/* Duration Summary */}
          <div className="text-xs font-semibold text-slate-500 bg-indigo-50/60 border border-indigo-100 rounded-xl px-3.5 py-2 flex items-center justify-between">
          ...
          <div className="border border-slate-100 rounded-2xl p-4 space-y-2 text-xs bg-white">
            <span className="font-bold text-slate-900 block mb-2 text-sm">Transparent Cost Breakdown</span>
            <div className="flex justify-between text-slate-600">
            ...
            <div className="flex justify-between text-slate-600">
              ...
              <span className="font-semibold text-slate-800">₹{securityDeposit}</span>
            </div>
            <div className="flex justify-between text-slate-600">
              <span>Platform & Protection Fee (10%)</span>
              <span className="font-semibold text-slate-800">₹{platformFee}</span>
            </div>

            <div className="flex justify-between pt-3 mt-2 border-t border-slate-200 text-sm font-extrabold text-slate-900">
              <span>Total Estimated Amount</span>
              <span className="text-indigo-600 text-base">₹{totalPayable}</span>
            </div>
          </div>
          ...
          {kycStatus !== 'verified' ? (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-900 space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-bold block">Identity Verification Required</span>
                  <p className="text-slate-600 leading-snug mt-0.5">
                    Your KYC is currently unverified. Complete 60-second DL and selfie verification before booking confirmation.
                  </p>
                </div>
              </div>
              <Button
                ...
                className="w-full bg-white border-amber-300 text-amber-800 hover:bg-amber-100/50 justify-center"
              >
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-800 flex items-center gap-2">
              ...
              <span>Identity Verified. You are eligible for 1-click keyless booking.</span>
            </div>
          )}
          ...
          <Button
            ...
            className="w-full py-3.5 text-base font-bold shadow-lg shadow-indigo-200"
          >
            Confirm & Pay ₹{totalPayable} via Escrow
          </Button>
